		...
		client.on("ready", () => {
			if(config.loadSlashsGlobal){
				...
				.then(slashCommandsData => {
					console.log(`${slashCommandsData.size} slashCommands ${`(With ${Array(slashCommandsData.map(d => d.options)).flat().length} Subcommands)`.green} Loaded for all: ${`All possible Guilds`.underline}`.brightGreen); 
				...
				client.guilds.cache.map(g => g).forEach((guild) => {
					try{
						...
						.then(slashCommandsData => {
							console.log(`${slashCommandsData.size} slashCommands ${`(With ${Array(slashCommandsData.map(d => d.options)).flat().length} Subcommands)`.green} Loaded for: ${`${guild.name}`.underline}`.brightGreen); 
						}).catch((e)=>console.log(e));
					}catch (e){
